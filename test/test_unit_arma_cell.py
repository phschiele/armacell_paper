...
from arma_cell.arma import ArmaCell
...
def test_AR() -> None:
    ...
    state = (tf.convert_to_tensor(np.zeros((batch_size, *cell.state_size)).astype(float)),)
...
def test_MA() -> None:
    ...
    cell.kernel = tf.convert_to_tensor(kernel)
    recurrent_kernel = np.arange(int(np.prod((cell.q, cell.units, cell.k, cell.k)))).reshape((cell.q, cell.units, cell.k, cell.k)).astype(float)
    cell.recurrent_kernel = tf.convert_to_tensor(recurrent_kernel)
    inputs = tf.convert_to_tensor(np.zeros((batch_size, k, p)).astype(float))
    state = (tf.convert_to_tensor(np.ones((batch_size, *cell.state_size)).astype(float)),)
    res = cell.call(inputs, state)
...
def test_MA_multiple_units() -> None:
    ...
    kernel = np.zeros((cell.p, cell.units, cell.k, cell.k)).astype(float)
    cell.kernel = tf.convert_to_tensor(kernel)
    recurrent_kernel = np.arange(int(np.prod((cell.q, 1, cell.k, cell.k)))).reshape((cell.q, 1, cell.k, cell.k)).astype(float)
...
def test_states() -> None:
    ...
    assert (np.array(output_state3[0, :, 0]) == np.array([263688.0, 300756.0, 337824.0, 374892.0])).all()
